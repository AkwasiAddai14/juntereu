...
import type { Locale } from '@/app/[lang]/dictionaries'; // define this type based on keys
  ...
  interface FlexpoolPageClientProps {
    id: string;
    lang: Locale;
    dashboard: any;
    components: any;
  ...
  export default function FlexpoolPage({ id, lang, dashboard }: FlexpoolPageClientProps) {
    ...
    if(!geauthoriseerd){
      ...
      useEffect(() => {
        ...
        if (!isSignedIn) {
          ...
          alert(`${dashboard.Flexpool.employees.ToastMessage1}`);
          return;
        }
  ...
  return (
    ...
    <h2 className="text-center text-2xl font-bold my-7">{dashboard.Flexpool.employees.FormFieldItems[0]}</h2>
      {shifts.length > 0 ? (
        <ScrollArea>
        ...
        <div className="lg:pl-96 h-full overflow-hidden">{dashboard.Flexpool.employees.FormFieldItems[1]}</div>
      )}
    </div>
  </>
 ) 
